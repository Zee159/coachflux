...
export function SessionReport({ sessionId, onClose }: SessionReportProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-lg max-w-4xl w-full my-4 sm:my-8 shadow-2xl print:shadow-none">
        {/* Header */}
            ...
            <section>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                {typeof goalPayload['goal'] === 'string' ? (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Your Goal</p>
                    <p className="text-gray-900">{goalPayload['goal']}</p>
                  </div>
                ) : null}
                {typeof goalPayload['why_now'] === 'string' ? (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Why Now</p>
                    <p className="text-gray-900">{goalPayload['why_now']}</p>
                  </div>
                ) : null}
                {Array.isArray(goalPayload['success_criteria']) ? (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Success Criteria</p>
                    <ul className="list-disc list-inside space-y-1">
                      {(goalPayload['success_criteria'] as string[]).map((criterion, idx) => (
                        <li key={idx} className="text-gray-900">{criterion}</li>
                      ))}
                    </ul>
                  </div>
                ) : null}
                {typeof goalPayload['horizon_weeks'] === 'number' ? (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Timeframe</p>
                    <p className="text-gray-900">{goalPayload['horizon_weeks']} weeks</p>
                  </div>
                ) : null}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                {typeof realityPayload['current_state'] === 'string' ? (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Current State</p>
                    <p className="text-gray-900">{realityPayload['current_state']}</p>
                  </div>
                ) : null}
                {Array.isArray(realityPayload['constraints']) && (realityPayload['constraints'] as unknown[]).length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Constraints</p>
                    <ul className="list-disc list-inside space-y-1">
                      {(realityPayload['constraints'] as string[]).map((constraint, idx) => (
                        <li key={idx} className="text-gray-900">{constraint}</li>
                      ))}
                    </ul>
                  </div>
                )}
                {Array.isArray(realityPayload['resources']) && (realityPayload['resources'] as unknown[]).length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Resources</p>
                    <ul className="list-disc list-inside space-y-1">
                      {(realityPayload['resources'] as string[]).map((resource, idx) => (
                        <li key={idx} className="text-gray-900">{resource}</li>
                      ))}
                    </ul>
                  </div>
                )}
                {Array.isArray(realityPayload['risks']) && (realityPayload['risks'] as unknown[]).length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Risks</p>
                    <ul className="list-disc list-inside space-y-1">
                      {(realityPayload['risks'] as string[]).map((risk, idx) => (
                        <li key={idx} className="text-gray-900">{risk}</li>
                      ))}
                    </ul>
                  </div>
                )}
              ...
              <div className="space-y-3">
                {(optionsPayload['options'] as Array<{ label: string; pros?: string[]; cons?: string[] }>).map((option, idx) => (
                  <div key={idx} className="bg-gray-50 rounded-lg p-4">
                    <p className="font-semibold text-gray-900 mb-2">{option.label}</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-sm">
                      ...
                      {option.cons !== undefined && option.cons !== null && option.cons.length > 0 && (
                        <div>
                          <p className="text-xs font-semibold text-red-700 uppercase mb-1">Cons</p>
                          <ul className="list-disc list-inside space-y-1 text-gray-700">
            ...
            <section>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                {typeof willPayload['chosen_option'] === 'string' ? (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Chosen Option</p>
                    <p className="text-gray-900 font-medium">{willPayload['chosen_option']}</p>
                  </div>
                ) : null}
                {Array.isArray(willPayload['actions']) && (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-2">Action Items</p>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                {typeof reviewPayload['summary'] === 'string' ? (
                  <div>
                    ...
                    <p className="text-gray-900">{reviewPayload['summary']}</p>
                  </div>
                ) : null}
                {typeof reviewPayload['alignment_score'] === 'number' && (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-1">Alignment Score</p>
                ...
                {Array.isArray(reviewPayload['value_tags']) && (reviewPayload['value_tags'] as unknown[]).length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 uppercase mb-2">Aligned Values</p>
              ...
              <div className="mt-4 space-y-4">
                ...
                {typeof reviewPayload['ai_insights'] === 'string' && (
                  <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
                    <p className="text-xs font-semibold text-blue-900 uppercase mb-1">Key Insights</p>
                    <p className="text-sm text-gray-800">{reviewPayload['ai_insights']}</p>
                  </div>
                )}

                {Array.isArray(reviewPayload['unexplored_options']) && (reviewPayload['unexplored_options'] as unknown[]).length > 0 && (
                  <div className="bg-purple-50 rounded-lg p-4">
                    <p className="text-xs font-semibold text-purple-900 uppercase mb-2">💡 Unexplored Options</p>
                ...
                {Array.isArray(reviewPayload['identified_risks']) && (reviewPayload['identified_risks'] as unknown[]).length > 0 && (
                  <div className="bg-orange-50 rounded-lg p-4">
                    <p className="text-xs font-semibold text-orange-900 uppercase mb-2">⚠️ Identified Risks</p>
                    <ul className="space-y-2">
                ...
                {Array.isArray(reviewPayload['potential_pitfalls']) && (reviewPayload['potential_pitfalls'] as unknown[]).length > 0 && (
                  <div className="bg-red-50 rounded-lg p-4">
                    <p className="text-xs font-semibold text-red-900 uppercase mb-2">🚨 Potential Pitfalls</p>
                    <ul className="space-y-2">
        ...
        {/* Footer */}
        <div className="border-t border-gray-200 p-4 sm:p-6 bg-gray-50 rounded-b-lg flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 print:hidden">
          <button
            onClick={handlePrint}
          ...
          <button
            onClick={onClose}
            className="w-full sm:w-auto px-4 py-2 text-sm bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 transition-colors"
          >
            Close
          </button>
        </div>
